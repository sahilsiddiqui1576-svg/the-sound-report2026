...
export default function EditorialCard({ entry, badge, size = "md" }: Props) {
  ...
  return (
    <Link href={href} className="group flex h-full flex-col border-r border-black/10 last:border-r-0 dark:border-white/10">
      <div className={`relative w-full ${aspect} shrink-0 overflow-hidden bg-neutral-200 dark:bg-neutral-800`}>
      ...
      <div className="flex flex-1 flex-col px-4 py-4 sm:px-5">
        <div className="flex items-center justify-between gap-3">
          ...
          {!isPlaylist && <span className="editorial-meta shrink-0">{format(new Date(frontmatter.publishDate), "MMM d")}</span>}
        </div>
        ...
        <span className="link-arrow mt-auto pt-5">{isPlaylist ? "Explore playlist" : "Read story"} <span aria-hidden>↗</span></span>
      </div>
    </Link>
  );
}
