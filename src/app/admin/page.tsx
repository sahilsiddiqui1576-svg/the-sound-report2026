...
export default function AdminDashboard() {
  ...
  async function refreshCounts(){const {data}=await createClient().from("content_entries").select("collection,draft");const next:Record<string,number>={};let d=0,p=0;data?.forEach(r=>{next[r.collection]=(next[r.collection]||0)+1;r.draft?d++:p++;});setCounts(next);setDrafts(d);setPublished(p);}
