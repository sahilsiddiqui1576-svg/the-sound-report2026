...
const blank = {
  title: "", slug: "", publish_date: new Date().toISOString(), featured: false, draft: true,
  category: "", tags: [], genre: [], language: [], month: "", year: new Date().getFullYear(),
  cover_image: "", cover_image_alt: "", excerpt: "", author: "", display_order: null,
  artist_name: "", artist_image: "", location: "", artist_links: {}, curator: "",
  spotify_url: "", apple_music_url: "", youtube_url: "", research_notes: "", tracks: [], body: "",
  seo_title: "", seo_description: "", seo_image: "", canonical_url: ""
};
...
export default function AdminCollection() {
  ...
  async function save() {
    ...
    const response = editing.id
      ? await fetch(`/api/admin/content/${editing.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
    ...
    else { setMessage("Saved."); setEditing(data); await load(); }
    setSaving(false);
  }
  ...
  async function upload(file: File) {
    ...
    const data = await response.json();
    if (response.ok) setEditing((x: Row) => ({ ...x, cover_image: data.url }));
    ...
    <main className="min-h-screen bg-neutral-50 dark:bg-[#0a0a0c]">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><Link href="/admin" className="text-xs font-semibold text-neutral-500">← Editorial Desk</Link><h1 className="mt-2 font-display text-4xl font-black">{isArtist ? "Artists" : meta.label}</h1></div>
          <button onClick={() => setEditing({ ...blank })} className="rounded-full bg-black px-5 py-3 text-sm font-bold text-white dark:bg-white dark:text-black">+ Add {isArtist ? "Artist" : meta.singularLabel}</button>
        </div>

        <div className="mt-8 grid gap-3">
          {loading ? <p className="text-sm text-neutral-500">Loading…</p> : rows.length === 0 ? <p className="rounded-2xl border border-dashed p-8 text-center text-sm text-neutral-500">No content yet. Add your first {isArtist ? "artist spotlight" : meta.singularLabel.toLowerCase()}.</p> : rows.map((row) => <div key={row.id} className="flex items-center justify-between gap-4 rounded-2xl border border-black/10 bg-white p-4 dark:border-white/10 dark:bg-white/[.04]"><div className="min-w-0"><p className="truncate font-semibold">{row.title}</p><p className="mt-1 text-xs text-neutral-500">{row.draft ? "Draft" : "Published"} · {row.slug}</p></div><div className="flex gap-2"><button onClick={() => setEditing(row)} className="rounded-lg border px-3 py-2 text-xs">Edit</button><button onClick={() => remove(row.id)} className="rounded-lg border px-3 py-2 text-xs text-red-600">Delete</button></div></div>)}
        </div>
...
function Editor({ value, setValue, onSave, onCancel, onUpload, saving, message, isArtist, isPlaylist }: { value: Row; setValue: any; onSave: () => void; onCancel: () => void; onUpload: (file: File) => void; saving: boolean; message: string; isArtist: boolean; isPlaylist: boolean }) {
  ...
  function setTrack(index: number, key: string, val: string) {
    const next = tracks.map((track: Row, i: number) => i === index ? { ...track, [key]: val } : track);
    set("tracks", next);
  }
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4"><div className="mx-auto my-8 max-w-4xl rounded-3xl bg-white p-6 shadow-2xl dark:bg-[#111114] sm:p-8">
    ...
    <div className="mt-7 grid gap-5 sm:grid-cols-2">
      ...
      {isArtist && <>
        ...
        <Field label="Artist image URL"><Input value={value.artist_image || ""} onChange={(e: any) => set("artist_image", e.target.value)} /></Field>
        <Field label="Artist website / social"><Input value={value.artist_links?.website || ""} onChange={(e: any) => set("artist_links", { ...(value.artist_links || {}), website: e.target.value })} /></Field>
      </>}
